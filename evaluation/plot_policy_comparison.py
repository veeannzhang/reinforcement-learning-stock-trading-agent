...
def plot_policy_comparison(tickers, mean_base, mean_high, std_base, std_high, model_name=None):
    """
    ...
    title_name_str = f" for {model_name}" if model_name else ""
    ax.set_ylabel('Mean Action Value (Logits)')
    ax.set_title(f'Comparison of Mean Actions: Baseline vs High VIX{title_name_str}')
    ...
    ax.set_title(f'Comparison of Action Uncertainty (Std): Baseline vs High VIX{title_name_str}')
    ax.set_xticks(indices)
    ax.set_xticklabels(tickers)
    ax.legend()
    ax.grid(axis='y', linestyle='--', alpha=0.7)
